...
export async function setup() {
  ...
  class DomainVitestWorkflowBuilder extends BaseBuilder {
    constructor(
      workingDir: string,
  ...
  await rm(join(cwd, ".workflow-data"), { recursive: true, force: true })
  const builder = new DomainVitestWorkflowBuilder(cwd, outDir)
  await builder.build()
