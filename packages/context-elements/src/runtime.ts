...
const tempRuntimeStoreSymbol = Symbol.for("ekairos.contextElements.tempRuntimeStore");
...
async function resolveRuntimeCredentials(
  ...
  if (!token) {
    throw new Error(
      [
        "context-elements runtime requires an Instant platform token",
        "(INSTANT_PERSONAL_ACCESS_TOKEN or INSTANT_PLATFORM_ACCESS_TOKEN).",
        "Add it to packages/context-elements/.env.local",
      ].join(" "),
    );
  }
...
export const runtimeConfig = configureRuntime<RuntimeEnv>({
  runtime: async (env, _domain, options) => {
    ...
    await options?.onProgress?.({
      state: "ready",
      progress: 100,
      message: `Context-elements runtime ready (temporary app: ${creds.title ?? creds.appId}).`,
    });
