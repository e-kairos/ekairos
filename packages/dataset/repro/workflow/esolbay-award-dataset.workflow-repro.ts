...
export const esolbayAwardDatasetReproDomain = domain(
...
export class EsolbayAwardDatasetReproRuntime extends EkairosRuntime<
  EsolbayAwardDatasetReproEnv,
  typeof esolbayAwardDatasetReproDomain,
  ReturnType<InstantAdminInit>
> {
  static [WORKFLOW_SERIALIZE](instance: EsolbayAwardDatasetReproRuntime) {
    return { env: instance.env }
  }

  static [WORKFLOW_DESERIALIZE](data: { env: EsolbayAwardDatasetReproEnv }) {
    return new EsolbayAwardDatasetReproRuntime(data.env)
  }

  protected getDomain() {
    return esolbayAwardDatasetReproDomain
  }

  protected async resolveDb(env: EsolbayAwardDatasetReproEnv) {
    const { init } = await import("@instantdb/admin")
    return init({
      appId: env.appId,
      adminToken: env.adminToken,
      schema: esolbayAwardDatasetReproDomain.toInstantSchema(),
      useDateObjects: true,
    } as any)
  }
}

export type EsolbayAwardDatasetReproInput = {
...
export type EsolbayAwardDatasetReproResult = {
  datasetId: string
  originalDatasetId: string
...
function getAzureProvider(): OpenAIProvider {
  ...
  const apiKey =
    readTrimmedEnv("AZURE_API_KEY") || readTrimmedEnv("AZURE_OPENAI_API_KEY")
  if (!apiKey) {
...
export async function esolbayAwardDatasetReproWorkflow(
  input: EsolbayAwardDatasetReproInput,
): Promise<EsolbayAwardDatasetReproResult> {
  "use workflow"
