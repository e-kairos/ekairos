...
import { contextDomain, type ContextItem } from "@ekairos/events"
...
// when: the runtime root includes contextDomain and the required business domain.
...
// then: a runtime with only contextDomain is rejected because it cannot satisfy
...
// Engine also needs contextDomain for persistence.
workspaceContext.react(trigger, {
  // @ts-expect-error runtime root domain must include contextDomain
