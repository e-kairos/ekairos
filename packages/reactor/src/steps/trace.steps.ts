...
export async function writeContextTraceEvents<Env extends ContextEnvironment = ContextEnvironment>(params: {
  ...
  try {
    ...
    if (db) {
      ...
      const byRun = new Map<string, ContextTraceEventWrite[]>();
      ...
      for (const [runId] of byRun) {
        ...
        try {
          ...
          const row = q?.reaction_trace_runs?.[0];
      ...
      for (const ev of params.events) {
        ...
        txs.push(
          db.tx.reaction_trace_events[lookup("key", key)].update({
            key,
            workflowRunId: runId,
            seq,
        ...
        if (ev.eventKind === "context.step" || ev.eventKind === "workflow.step") {
          ...
          spanTxs.push(
            db.tx.reaction_trace_spans[lookup("spanId", spanId)].update({
              spanId,
              parentSpanId: ev.parentSpanId,
              workflowRunId: runId,
              executionId: ev.executionId,
      ...
      for (const [runId, events] of byRun) {
        ...
        await db.transact([
          db.tx.reaction_trace_runs[lookup("workflowRunId", runId)].update({
            workflowRunId: runId,
            orgId: orgId || undefined,
            projectId: projectId || undefined,
            firstEventAt,
  ...
  if (!projectId) {
    if (strict) throw new Error("[context/trace] Missing projectId (ekairosConfig or EKAIROS_PROJECT_ID)");
    return;
  }
