...
async function createRuntimeOps<Context>(
  ...
  const { db } = runtime
  const { InstantStore } = await import("@ekairos/events/instant")
  ...
  return {
    ...
    openExecution: async ({ contextIdentifier, triggerEvent }) => {
      ...
      await instrumentedDb.transact([
        ...
        instrumentedDb.tx.event_executions[executionId].link({ input: triggerId }),
        instrumentedDb.tx.event_executions[executionId].link({ output: reactionId }),
        instrumentedDb.tx.event_items[triggerId].link({ reactions: reactionId }),
...
async function createWorkflowOps<Context>(
  ...
  return {
    ...
    createContextStep: async ({ executionId, reactionEventId, iteration }) =>
      await createContextStep({ runtime, executionId, reactionEventId, iteration }),
    updateContextStep: async (params) =>
