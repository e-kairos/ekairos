...
export async function verifyEventDomainRun(
  ...
  const linkedExecutionInput = asRecord(execution.input)
  const linkedExecutionOutput = asRecord(execution.output)
  if (asString(linkedExecutionContext.id)) {
    expect(asString(linkedExecutionContext.id)).toBe(params.contextId)
  }
  if (params.triggerEventId && asString(linkedExecutionInput.id)) {
    expect(asString(linkedExecutionInput.id)).toBe(params.triggerEventId)
  }
  if (asString(linkedExecutionOutput.id)) {
    expect(asString(linkedExecutionOutput.id)).toBe(params.reactionEventId)
  }
  ...
  if (triggerItem && reactionItem) {
    const triggerReactions = Array.isArray(triggerItem.reactions)
      ? triggerItem.reactions as AnyRecord[]
      : []
    expect(triggerReactions.some((item) => asString(item.id) === params.reactionEventId)).toBe(true)
    expect(asString(asRecord(reactionItem.trigger).id)).toBe(asString(triggerItem.id))
  }
